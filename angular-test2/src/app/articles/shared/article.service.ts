...
import { Article, PagedList } from './index';
...
export class ArticlesService {
    ...
    public getArticles(params?: {}): Observable<PagedList<Article>> {
        ...
        if (price) {
            search.set('price', price);
        }

        return this.http.get(`${this.baseUrl}/articles`, this.getCommonRequestOption( { search } ))
            .map(this.extractData)
            .catch(this.handleError);
    }

    private extractData(res: Response): PagedList<Article> {
        let json: any = res.json();
        let data: PagedList<Article> = new PagedList<Article>();
        data.totalCount = json.totalCount;
        data.items = Article.parseCollection(json.items);
        return data;
    }
